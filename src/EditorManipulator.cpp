...
/// Constructor.
EditorManipulator::EditorManipulator( int flags )
   : inherited( flags ),
     _distance( 1. ),
     _trackballSize( 0.8 )
{
...
/// Constructor.
EditorManipulator::EditorManipulator( const EditorManipulator& om, const CopyOp& copyOp )
...
void EditorManipulator::setByMatrix( const osg::Matrixd& matrix )
{
...
/** Set the position of the manipulator using a 4x4 matrix.*/
void EditorManipulator::setByInverseMatrix( const osg::Matrixd& matrix )
{
...
osg::Matrixd EditorManipulator::getMatrix() const
{
...
osg::Matrixd EditorManipulator::getInverseMatrix() const
{
...
// doc in parent
void EditorManipulator::setTransformation( const osg::Vec3d& eye, const osg::Quat& rotation )
{
...
// doc in parent
void EditorManipulator::getTransformation( osg::Vec3d& eye, osg::Quat& rotation ) const
{
...
// doc in parent
void EditorManipulator::setTransformation( const osg::Vec3d& eye, const osg::Vec3d& center, const osg::Vec3d& up )
{
    Vec3d lv( center - eye );
...
// doc in parent
void EditorManipulator::getTransformation( osg::Vec3d& eye, osg::Vec3d& center, osg::Vec3d& up ) const
{
    center = _center;
    eye = _center + _rotation * osg::Vec3d( 0., 0., _distance );
...
void EditorManipulator::setHeading( double azimuth )
{
...
double EditorManipulator::getHeading() const
{
    CoordinateFrame coordinateFrame = getCoordinateFrame( _center );
...
void EditorManipulator::setElevation( double elevation )
{
    CoordinateFrame coordinateFrame = getCoordinateFrame( _center );
    Vec3d localUp = getUpVector( coordinateFrame );
    Vec3d localRight = getSideVector( coordinateFrame );
...
double EditorManipulator::getElevation() const
{
    CoordinateFrame coordinateFrame = getCoordinateFrame( _center );
    Vec3d localUp = getUpVector( coordinateFrame );
    localUp.normalize();
...
// doc in parent
bool EditorManipulator::handleMouseWheel( const GUIEventAdapter& ea, GUIActionAdapter& us )
{
...
// doc in parent
bool EditorManipulator::performMovementLeftMouseButton( const double eventTimeDelta, const double dx, const double dy )
{
    // rotate camera
    if( getVerticalAxisFixed() )
...
// doc in parent
bool EditorManipulator::performMovementMiddleMouseButton( const double eventTimeDelta, const double dx, const double dy )
{
    // pan model
...
// doc in parent
bool EditorManipulator::performMovementRightMouseButton( const double eventTimeDelta, const double /*dx*/, const double dy )
{
    // zoom model
...
void EditorManipulator::performMovementKeyW(const double eventTimeDelta, const double /*dx*/, const double dy)
{
	EditorManipulator::moveForward(dy * getThrowScale( eventTimeDelta ));
}


bool EditorManipulator::performMouseDeltaMovement( const float dx, const float dy )
{
    // rotate camera
    if( getVerticalAxisFixed() )
        rotateWithFixedVertical( dx, dy );
...
void EditorManipulator::applyAnimationStep( const double currentProgress, const double prevProgress )
{
...
bool EditorManipulator::startAnimationByMousePointerIntersection(
...
void EditorManipulator::OrbitAnimationData::start( const osg::Vec3d& movement, const double startTime )
{
...
void EditorManipulator::rotateTrackball( const float px0, const float py0,
...
void EditorManipulator::rotateWithFixedVertical( const float dx, const float dy )
{
    CoordinateFrame coordinateFrame = getCoordinateFrame( _center );
    Vec3d localUp = getUpVector( coordinateFrame );
...
void EditorManipulator::rotateWithFixedVertical( const float dx, const float dy, const Vec3f& up )
{
...
void EditorManipulator::panModel( const float dx, const float dy, const float dz )
{
 ...
 */
void EditorManipulator::zoomModel( const float dy, bool pushForwardIfNeeded )
{
    // scale
    float scale = 1.0f + dy;
 ...
 * (-1.0 ... 1.0)
 */
void EditorManipulator::trackball( osg::Vec3d& axis, float& angle, float p1x, float p1y, float p2x, float p2y )
{
    /*
 ...
 */
float EditorManipulator::tb_project_to_sphere( float r, float x, float y )
{
    float d, t, z;
...
osgUtil::SceneView::FusionDistanceMode EditorManipulator::getFusionDistanceMode() const
{
    return osgUtil::SceneView::USE_FUSION_DISTANCE_VALUE;
}

/** Get the FusionDistanceValue. Used by SceneView for setting up stereo convergence.*/
float EditorManipulator::getFusionDistanceValue() const{return _distance;}


/** Set the center of the manipulator. */
void EditorManipulator::setCenter( const Vec3d& center ){_center = center;}


/** Get the center of the manipulator. */
const Vec3d& EditorManipulator::getCenter() const{return _center;}


/** Set the rotation of the manipulator. */
void EditorManipulator::setRotation( const Quat& rotation ){_rotation = rotation;}


/** Get the rotation of the manipulator. */
const Quat& EditorManipulator::getRotation() const{return _rotation;}


/** Set the distance of camera to the center. */
void EditorManipulator::setDistance( double distance ){_distance = distance;}


/** Get the distance of the camera to the center. */
double EditorManipulator::getDistance() const{return _distance;}


/** Set the size of the trackball. Value is relative to the model size. */
void EditorManipulator::setTrackballSize( const double& size )
{
    /*
...
void EditorManipulator::setWheelZoomFactor( double wheelZoomFactor )
{
...
void EditorManipulator::setMinimumDistance( const double& minimumDistance, bool relativeToModelSize )
{
...
/** Get the minimum distance of the eye point from the center
    before the center is pushed forward.*/
double EditorManipulator::getMinimumDistance( bool *relativeToModelSize ) const
{
    if( relativeToModelSize ){*relativeToModelSize = getRelativeFlag( _minimumDistanceFlagIndex );}
...
void EditorManipulator::moveForward( const double distance )
{
   ...
   // get current transformation
    ...
    osg::Vec3d eye;
    eye = prevEye + EditorManipulator::getRotation() * Vec3d( 0., 0., -distance );
    
    EditorManipulator::setTransformation(eye,EditorManipulator::getRotation());
}
